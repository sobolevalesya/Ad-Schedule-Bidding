...
function main() {
    ...
    var spreadsheetUrl = "PASTE_YOUR_LINK_TO_SREADSHEET_HERE";
    ...
    while (campaignIterator.hasNext()) {
        ...
        for (var i = 0; i < timesAndModifiers.length; i++) {
            if (existingSchedules.indexOf(
                    ...
                    " | "+Utilities.formatString(" % .2 f", (timesAndModifiers[i][3] + 1)) + " | "+campaign.getId()) >
                -1) {
...
function checkAndRemoveAdSchedules(campaignIds, timesAndModifiers) {
    ...
    for (var j = 0; j < timesAndModifiers.length; j++) {
        wantedSchedules.push(timesAndModifiers[j][0] + " | "+(timesAndModifiers[j][1]) + " | "+timesAndModifiers[j][2] + " | "+Utilities.formatString(" % .2 f", timesAndModifiers[j][3] + 1));
    }
    ...
    for (var i = 0; i < chunkedArray.length; i++) {
        ...
        while (adScheduleIterator.hasNext()) {
            ...
            var key = adSchedule.getStartHour() + " | "+adSchedule.getEndHour() + " | "+adSchedule.getDayOfWeek() + " | "+Utilities.formatString(" % .2 f", adSchedule.getBidModifier());
